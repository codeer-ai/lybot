...
def search_gazettes(
    ...
    """
    logger.info(
        f"Searching gazettes - dates: {date_start} to {date_end}, keywords: {keywords}"
    )
...
def extract_voting_records_from_pdf(
    pdf_url: str, bill_identifier: Optional[str] = None
) -> Dict[str, Any]:
    """
...
def parse_voting_records(
    content: str, bill_identifier: Optional[str] = None
) -> Dict[str, Any]:
    """
    ...
    for section in sections:
        ...
        for line in lines:
            ...
            if current_vote_type:
                ...
                clean_line = re.sub(
                    r"(贊成者|反對者|棄權者|贊成委員|反對委員|棄權委員)[:：]?", "", line
                )
                ...
                for name in names:
                    ...
                    if len(name) >= 2 and len(name) <= 4 and not name.isdigit():
                        if all(
                            c not in name for c in ["表決", "議案", "委員會", "主席"]
                        ):
...
def find_voting_records_for_bill(
    ...
    gazettes = search_gazettes(
        date_start=date_start, date_end=date_end, keywords=bill_name
    )
